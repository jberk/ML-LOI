...
df1 = pd.read_csv('56d_MICE_imputed_dataset_1-Copy1.csv')
df2 = pd.read_csv('56d_MICE_imputed_dataset_2-Copy1.csv')
df3 = pd.read_csv('56d_MICE_imputed_dataset_3-Copy1.csv')
df4 = pd.read_csv('56d_MICE_imputed_dataset_4-Copy1.csv')
df5 = pd.read_csv('56d_MICE_imputed_dataset_5-Copy1.csv')
...
print(df2['Long_stay'].value_counts())
...
def compute_balanced_weights(y):
    ...
    total_samples = len(y)
    return {cls: total_samples / (2.0 * count) for cls, count in class_counts.items()}
...
for df in df_list:
    ...
    df_train_years = df[df['year'] <= 2017]

    df_train_years = df_train_years.drop(columns = ['year'])

    #print(df_train_years.info())

    df_test_years = df[df['year'] >= 2018]

    df_test_years = df_test_years[df_test_years['year'] <= 2019]
    ...
    cat_features = df_train_years.select_dtypes(include=['category']).columns.tolist()
   ...
   # param_grid = {
   #     'iterations': [1000], #1000 is default value
   #     'learning_rate': [0.03, 0.1], #0.03 is default value, added in faster learning rate
   #     'depth': [6, ], #6 is default value
   #     'l2_leaf_reg': [3,10], #3 is default value, higher value = more regularization, less overfitting
    #    'class_weights': ['balanced']
    #    #'border_count':[128] #speed up training
   # }


    param_grid = {
    'iterations': [1000], #1000 is default value
    'learning_rate': [0.01, 0.03, 0.1], #0.03 is default value
    'depth': [6, 8,10], #6 is default value
    'l2_leaf_reg': [1, 3, 10], #3 is default value, higher value = more regularization, less overfitting
    'class_weights': ['balanced']}
    ...
    for fold_number, (train_index, test_index) in enumerate(outer_cv.split(df_model, labels), start=1):
        ...
        for i, class_weight in enumerate(adjusted_param_grid['class_weights']):
            if class_weight == 'balanced':
                balanced_weights = compute_balanced_weights(y_train)
                adjusted_param_grid['class_weights'][i] = balanced_weights
                print(f"Using balanced weights for this fold: {balanced_weights}")
    ...
    final_model = CatBoostClassifier(cat_features=cat_features, **final_params)
    ...
    joblib.dump(final_model,f"cat_56d_avgp_MICE_new_{j}_bestparam.joblib")
    print('Model saved!')
...
pooled_results.to_csv('Cat_MICE_56d_Lim.csv')
