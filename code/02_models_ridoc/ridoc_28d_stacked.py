...
for df in df_list:
    ...
    rf_model = joblib.load(f"brf_28d_avgp_MICE_new_{j}.joblib")
    gbm_model = joblib.load(f"gbm_28d_avgp_MICE_new_{j}.joblib")
    log_model = joblib.load(f"log_28d_avgp_MICE_new_{j}.joblib")
    catboost_model = joblib.load(f"cat_28d_avgp_MICE_new_{j}.joblib")
...
avg_metrics_df.to_csv('Stacked 28d RIDOC MICE.csv')
