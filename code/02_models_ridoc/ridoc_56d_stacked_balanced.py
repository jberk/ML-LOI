...
for df in df_list:
    ...
    #loaded instead of training each model separately given had this in previous pipeline
    #but use clone to create new instance of the model with same parameters but no training data (the issue w/ previous code draft)

    rf_model = joblib.load(f"brf_56d_avgp_MICE_new_{j}.joblib")
    gbm_model = joblib.load(f"gbm_56d_avgp_MICE_new_{j}_apr.joblib")
    log_model = joblib.load(f"log_56d_avgp_MICE_new_{j}.joblib")
    catboost_model = joblib.load(f"cat_56d_avgp_MICE_new_{j}_bestparam.joblib")
    ...
    for i, base_model in enumerate(base_models):
        ...
        model_clone = clone(base_model) #clone
    ...
    # Tune threshold here
    y_pred_test = (y_prob_test >= 0.05).astype(int) #optimal threshold
    ...
    preds = calibrated_final_meta.predict_proba(test_meta_features)[:, 1]
...
avg_metrics_df.to_csv('Log Stacked 56d RIDOC MICE.csv')
...
precision, recall, thresholds = precision_recall_curve(y_test, mean_preds)
pr_auc = auc(recall, precision)

# align lengths (drop last precision/recall point)
df = pd.DataFrame({
...
# save to CSV
df.to_csv("Stacked_RIDOC_56d_NewMice_precision_recall_curve.csv", index=False)

#print("Saved PR curve. AUC:", pr_auc)
